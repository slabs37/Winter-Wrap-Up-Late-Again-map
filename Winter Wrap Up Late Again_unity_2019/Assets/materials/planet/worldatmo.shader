Shader "Custom/Atmosphere"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float fresnellight = pow(dot(-_WorldSpaceLightPos0, IN.viewDir)*(1-_Shadow), _SunPow);
            float fresnelnorm = dot(IN.worldNormal, IN.viewDir);
            float fresnel = saturate(fresnellight * fresnelnorm);
            fresnel = pow(fresnel, _FresnelPOW);

            o.Alpha = fresnel;
            o.Emission = _Color*_Emissive*fresnel;
        }
        ENDCG
    }
    FallBack "Diffuse"
