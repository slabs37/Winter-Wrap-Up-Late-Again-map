	...
	SubShader {
		Pass{
			...
			vertexOutput vert( vertexInput v ) {
                ...
                v.vertex = rotatePoint(float4(0, (_Rotation*1.5)+(sin(_Time.y)*0.05), 0, 0), v.vertex.xyz).xyzz; // from Math.cginc
			...
			float4 frag( vertexOutput i ): COLOR {
				...
				return float4( tex.xyz * lightFinal * _Color.xyz, 0);
			}
