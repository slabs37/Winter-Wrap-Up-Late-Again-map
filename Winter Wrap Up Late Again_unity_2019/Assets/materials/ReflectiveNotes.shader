Shader "Custom/CustomReflectiveNote"
{
    Properties
    {
        ...
        */
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 val = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, simplex(i.coord) + coords);
                finalColor.xyz = DecodeHDR(val, unity_SpecCube0_HDR);
                finalColor = saturate(finalColor+0.3);
                finalColor.w = 1.0;

                float shadowf = 1-(saturate(dot(-_WorldSpaceLightPos0, i.coord)));

                // Return some basic shading based on the note color
                float lighting = pow(i.localPos.y + 0.8, 4);
                return float4(Color *shadowf * finalColor, 0);
            }
            ENDCG
        }
         Pass
